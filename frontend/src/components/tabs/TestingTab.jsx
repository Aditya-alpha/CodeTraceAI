...
export default function TestingTab({ repoId, initialRouteId = null }) {
  ...
  // Active view inside route: 'scenarios' or 'code'
  ...
  // Load all tests and routes
  const loadData = async () => {
    try {
      setLoading(true);
      const data = await getRepoTests(repoId);
      ...
      if (data.routes && data.routes.length > 0) {
        const targetRoute = initialRouteId
          ? data.routes.find((r) => r._id === initialRouteId) || data.routes[0]
          : data.routes[0];
  ...
  useEffect(() => {
    ...
    if (existing) {
      ...
      // Fetch or auto-generate on demand
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-6 rounded-2xl glass-panel flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-xs font-mono font-medium text-sky-400 mb-1">
            ...
            <span>PHASE 2: AI TEST GENERATION (JEST + SUPERTEST)</span>
          </div>
          <h2 className="text-xl font-bold text-slate-100 flex items-center gap-2">
            <span>Automated Test Code Synthesis</span>
            <span className="text-xs font-mono px-2 py-0.5 rounded-full bg-sky-500/10 text-sky-400 border border-sky-500/20">
              No Execution
            </span>
          </h2>
          <p className="text-xs text-slate-400 mt-0.5">
            AST-driven scenario enumeration synthesizes verifiable Jest + Supertest suites. Low-confidence tests are flagged for developer review.
          </p>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-3 px-4 py-2 rounded-xl bg-[#080c14]/60 border border-white/[0.04] text-xs">
            ...
            <div className="w-[1px] h-6 bg-white/10" />
            <div>
              <span className="text-slate-500 block text-[10px] uppercase font-mono">Reviewed</span>
              <span className="font-bold text-emerald-400">{stats.reviewedTests || 0}</span>
            </div>
          </div>
          ...
          <button
            ...
            className="px-4 py-2.5 rounded-xl bg-sky-500 hover:bg-sky-400 disabled:opacity-50 text-white text-xs font-semibold flex items-center gap-1.5 transition-colors shadow-lg shadow-sky-500/10"
          >
            <Sparkles className={`w-3.5 h-3.5 ${batchGenerating ? 'animate-spin' : ''}`} />
            <span>{batchGenerating ? 'Generating All...' : 'Generate All Tests'}</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 min-h-[600px]">
        ...
        <div className="lg:col-span-4 rounded-2xl glass-panel p-4 flex flex-col space-y-3 h-[720px]">
        ...
        {/* Right Column: Test Plan Viewer & Code Synthesis (8 Cols) */}
        <div className="lg:col-span-8 rounded-2xl glass-panel p-6 flex flex-col h-[720px] overflow-hidden">
          {selectedRoute ? (
            <div className="flex-1 flex flex-col h-full space-y-4">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-white/[0.06]">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    disabled={generating}
                    className="px-3 py-1.5 rounded-lg bg-[#080c14] border border-white/[0.08] hover:border-sky-500/40 text-slate-300 hover:text-white text-xs font-medium flex items-center gap-1.5 transition-colors"
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium border flex items-center gap-1.5 transition-colors ${
                      currentTestPlan?.isReviewed
                        ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40'
                        : 'bg-[#080c14] border-white/[0.08] text-slate-400 hover:text-slate-200'
                    }`}
                  >
                    <FileCheck className="w-3.5 h-3.5" />
                    <span>{currentTestPlan?.isReviewed ? 'Reviewed' : 'Mark Reviewed'}</span>
                  </button>
              ...
              {/* View Switcher: Scenarios Matrix vs Jest Code */}
              <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-2 p-1 rounded-xl bg-[#080c14]/80 border border-white/[0.06] w-fit">
                  <button
                  ...
                  >
                    ...
                    <span>AST Scenarios Checklist ({currentTestPlan?.scenarios?.length || 0})</span>
                  </button>
                </div>

                {/* Scenario Metric Indicators */}
                <div className="flex items-center gap-2 text-[11px]">
                  <span className="text-emerald-400 flex items-center gap-1">
                    <ShieldCheck className="w-3 h-3" />
                    <span>{highConfidenceCount} AST-Verified</span>
                  </span>
              ...
              {/* VIEW 1: SCENARIOS CHECKLIST MATRIX */}
              {viewMode === 'scenarios' && (
            ...
            <div className="flex-1 flex items-center justify-center text-slate-500 text-xs">
              Select an endpoint from the left to view and synthesize test code.
            </div>
          )}
        </div>
      </div>
